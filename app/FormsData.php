...
class FormsData extends Eloquent {
	...
	public function save(array $opts = array('table'=>"")) {
		...
		//$txtDebug .= PHP_EOL."  \$dbconn - ".print_r($dbconn->toArray(),1);
		$txtDebug .= PHP_EOL."  \$dbconn: pretending - ".var_export($dbconn->pretending(),1);
		...
		$queryy = $this->newQuery();
		$fff = $this->fresh();
		$txtDebug .= PHP_EOL." \$query - ".print_r($query->toSql(), 1);
		$txtDebug .= PHP_EOL." \$queryy - ".print_r($queryy->toSql(), 1);
