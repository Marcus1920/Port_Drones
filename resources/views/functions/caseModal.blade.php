...
//      setTimeout(function(){AutoReload();}, 30000);
		$("#add_officer").click(function (ev) {
			console.log("#add_officer.click")
			var elCell = $("#CreateCaseAgentForm #investigation_cell");
			if (elCell.length == 0) elCell = $(ev.target).closest(".container").find("#investigation_cell");
			var elEmail = $("#CreateCaseAgentForm #investigation_email");
			if (elEmail.length == 0) elEmail = $(ev.target).closest(".container").find("#investigation_email");
			$("#CreateCaseAgentForm #officers").val(0);
			elCell.val("");
			elEmail.val("");
			$("#CreateCaseAgentForm #officers").val(0);
			$("#txtOfficer").removeClass("hidden");
			elCell.removeAttr("disabled");
			elEmail.removeAttr("disabled");
		});
		$("#officers").change(function (ev) {
			console.log("#offiecers.change()");
			var officer = $("#CreateCaseAgentForm #officers option:selected").val();
			if (officer == undefined) officer = $(ev.target).closest(".container").find("#officers option:selected").val();

			var elCell = $("#CreateCaseAgentForm #investigation_cell");
			if (elCell.length == 0) elCell = $(ev.target).closest(".container").find("#investigation_cell");
			var elEmail = $("#CreateCaseAgentForm #investigation_email");
			if (elEmail.length == 0) elEmail = $(ev.target).closest(".container").find("#investigation_email");
			elCell.prop("disabled");
			elEmail.prop("disabled");
			...
			if (officer == 0) {
				...
				$.ajax({
					...
					success: function (data) {
						console.log("success getting officers: data - ",data);
						if (data[0] !== null) {
							elCell.val(data[0].cellphone);
							elEmail.val(data[0].email);
							elCell.prop("disabled",true);
							elEmail.prop("disabled", true);
						}
						else {
							$("#modalCase #name").val('');
						}
		...
		$("#reporters").change(function (ev) {
			console.log("#reporters.change(ev) this - ",this);
			console.log("  ev - ",ev);
			var reporter = $("#CreateCaseAgentForm #reporters option:selected").val();
			console.log("  reporter (A): - ",reporter,", == '' - ",(reporter == ""),", !reporter - ",(!reporter));
			if (reporter == "" || !reporter) reporter = $(ev.target).closest(".container").find("#reporters option:selected").val();
			console.log("  reporter (B): - ",reporter,", == '' - ",(reporter == ""),", !reporter - ",(!reporter));
			if (reporter == "" || !reporter) reporter = $(ev.target).find("option:selected").val();
			console.log("  reporter (C): - ",reporter,", == '' - ",(reporter == ""),", !reporter - ",(!reporter));
				var elReporters = this;

			var elCell = $("#CreateCaseAgentForm #reporter_cell");
			if (elCell.length == 0) elCell = $(ev.target).closest(".container").find("#reporter_cell");
			var elEmail = $("#CreateCaseAgentForm #reporter_email");
			if (elEmail.length == 0) elEmail = $(ev.target).closest(".container").find("#reporter_email");
			elCell.prop("disabled");
			elEmail.prop("disabled");
			//alert(officer);
      if (reporter == 0) {
				$("#wReporterName").removeClass("hidden");
				elCell.removeAttr("disabled");
				elEmail.removeAttr("disabled");
				$("#wReporterName").val("");
				elCell.val("");
				...
				$("#wReporterName").addClass("hidden");
        $.ajax({
          type: "GET",
          dataType: "json",
          url: "{!! url('/getReporter/"+reporter+"') !!}",
          success: function (data) {
            console.log("success getting reporter: data - ",data);
            if (data.length > 0) {
              elCell.val(data[0].cellphone);
              elEmail.val(data[0].email);
              elCell.prop("disabled",true);
		...
		console.log("#add_reporter.click()");
			var elReporter = $("#CreateCaseAgentForm #reporters");
			if (elReporter.length == 0) elReporter = $(ev.target).closest(".container").find("#reporters");
			console.log("  elReporter - ",elReporter);
			elReporter[0].selectedIndex = 0;
			$("#CreateCaseAgentForm #reporter_name").val("");
			$("#CreateCaseAgentForm #reporter_cell").val("");
			$("#CreateCaseAgentForm #reporter_email").val("");
			$("#CreateCaseAgentForm #reporter").val(0);
			$("#wReporterName").removeClass("hidden");
			$("#wReporterName").val("");
			$("#CreateCaseAgentForm #reporter_name").removeAttr("disabled");
			$("#CreateCaseAgentForm #reporter_cell").removeAttr("disabled");
			$("#CreateCaseAgentForm #reporter_email").removeAttr("disabled");
		});
		...
		$("#CreateCaseAgentForm #case_type").change(function (ev) {
			console.log("#CreateCaseAgentForm #case_type changing");
		...
		$("#cornford #case_type").change(function (ev) {
			console.log("#cornford #case_type changing: ev - ",ev);
			$.get("{{ url('/api/dropdownCaseType/sub_case_type/case_type')}}",
				{option: $(this).val()},
				function (data) {
					if ($.isArray(data)) {
					...
					if ($.isPlainObject(data)) {
						$("#sub_case_type_id").removeClass("hidden");
						//var elKid = $(ev.target).siblings();
						var elKid = $(ev.target).closest("#case_sub_type");
						console.log("elKid A - ", elKid);
						elKid = $(ev.target).closest(".container").find("#case_sub_type");
						console.log("elKid B - ", elKid);
						elKid.empty();
						elKid.append("<option value='0'>Select Case Type</option>");
						$.each(data, function (key, element) {
							elKid.append("<option value=" + key + ">" + element + "</option>");
						});
					}
				});
		});
...
// new  dataTable
		...
		var oTable = $('#casesTable').DataTable({
			...
			"columns": [
				...
				{data: 'status', name: 'cases_statuses.name'},
      ...
      /*"sAjaxSource": "{!! url('/resolved-cases-list/')!!}",
       ...
       "success": fnCallback
		...
		console.log("Look, Im getting called again at line 1116 in resources/views/functions/caseModal.blade.php");
		var pendingreferralCasesTable = $('#pendingreferralCasesTable').DataTable({
			...
			"aoColumnDefs": [
				...
				, {"sClass": "tc", "aTargets": [5]}
			]
		});

		function btnSeens() {
			$(".btnSeen").off("click");
			$(".btnSeen").on("click", function (ev) {
				console.log("btnSeen.click(ev) ev - ", ev);
				var el = ev.currentTarget;
				var cid = ev.currentTarget.id;
				var uid = {{ \Auth::check() ? \Auth::id() : -1 }};
				console.log("  cid - " + cid);
				cid = cid.replace("btnSeen_", "");
				var url = "{{ url("api/switchseen") }}" + "?cid=" + cid + "&uid=" + uid;
				console.log("  cid - " + cid + ", uid - " + uid + ", url - " + url);
				$.ajax(url, {
					'complete': function (xhr, status) {
						console.log("ajax.complete(xhr, status) xhr - ", xhr, ", status - ", status);
						var img = "{{ asset('images') }}/bell_" + xhr.responseJSON.img + ".png";
						console.log("  img - ", img);
						console.log("  el - ", el);
						el.src = img;
					}
		...
		pendingreferralCasesTable.on("draw", function (ev) {
			btnSeens();
		});
		window.setInterval(function () {
			//console.log("pendingreferralCasesTable - ",pendingreferralCasesTable);
			pendingreferralCasesTable.ajax.reload(function () {
				btnSeens();
			}, false);
		}, 5000);
...
//      $('#modalCase').modal('toggle');
		...
		$.ajax({
			...
			success: function (data) {
				if (data == "ok") {
					console.log("accepting case went well");
					document.getElementById("acceptCase").style.display = "none";
					document.getElementById("acceptedCase").style.display = "block";
//$("#side_contents6").css("display", "block");
					$(".panel-heading .wAlert").html('<div class="alert alert-success alert-icon">Well done! You successfully accepted case ' + id + '<i class="icon">&#61845;</i><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button></div>');
					HoldOn.close()
				}
			}
		})
